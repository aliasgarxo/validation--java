...
$(document).ready(function() {
	// Start : Create OU
	// On-Click handler for Create OU (save_ou) Button
	$("button.btn-create-ou").click(function(e) {
		// Post Form if data is valid
		var validator = $("#frmLandingZoneOU").validate();
		...
		var modalMessage;
		modalMessage = "Create Organizational Unit";
		$('#mfaExpiryDiv').show();
		$('.btn-proceed').show();
		...
		modalPopup.find('.modal-title').text("Create New Organizational Unit");
	...
	$("#infraPopupModal").on('show.bs.modal', function(e) {
		var btnVal = $("button.btn-create-ou").attr("value");
	...
	$("button.btn-proceed").click(function(e) {
		try {
			...
			if (isValid) {
				$("#cover-spin").show();
				$('#frmLandingZoneOU').attr("action", "/auth/landingzone?save_ou");
				$("#frmLandingZoneOU")[0].submit();
			}
			else {
