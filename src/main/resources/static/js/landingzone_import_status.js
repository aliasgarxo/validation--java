...
$(document).ready(function() {
	...
	$("button.btn-update-import-accounts").click(function(e) {
		// Post Form if data is valid
		var validator = $("#frmLandingZoneAccountStatus").validate();
		...
		var modalMessage;
		modalMessage = "Update Accounts Status";
		$('#mfaExpiryDiv').show();
		$('.btn-proceed').show();
		...
		modalPopup.find('.modal-title').text("Updated AWS Accounts Status");
	...
	// On-Click handler for Popup Modal Update Import Status Button
	$("button.btn-proceed").click(function(e) {
		try {
			...
			if (isValid) {
				$("#cover-spin").show();
				$('#frmLandingZoneAccountStatus').attr("action", "/auth/landingzone/accounts?update_import_accounts");
				$("#frmLandingZoneAccountStatus")[0].submit();
			}
			else {
