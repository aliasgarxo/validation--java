...
$(document).ready(function() {
	...
	$("button.btn-add-ou-config-rules").on("click", { param: "add-ou-config-rules" }, btnClickHandler);

	/*// On-Click handler for 'Fetch' Button - Fetch All AWS Config Rules
	$("button.btn-fetch-ou-config-rules").on("click", { param: "fetch-ou-config-rules" }, btnClickHandler);*/

	// On-Click handler for 'Remove' Button - Remove Config Rule in AWS
	$("button.btn-remove-ou-config-rule").on("click", { param: "remove-ou-config-rule" }, btnClickHandler);
	...
	$("#infraPopupModal").on('show.bs.modal', function(e) {
		...
		if (btnVal && btnVal.length > 0) {
			...
			modalPopup.find('.modal-message').text("Invalid Config Rule Details");
...
function btnClickHandler(e) {
	...
	if (e.data.param === "add-ou-config-rules" && nSelectedCount === 0) {
